...
public class FunVideoView extends RelativeLayout implements IFunSDKResult {
    ...
    private SwitchViewListener mOnSwitchViewListener = null;
	...
	public void setFishEye(boolean enable) {
		mIsFishEyeEnable = enable;
	}

	public boolean ismIsFishEyeEnable() {
		return mIsFishEyeEnable;
	}

	public void setmOnSwitchViewListener(SwitchViewListener switchViewListener){
		this.mOnSwitchViewListener = switchViewListener;
	}
	...
	private void switchSurfaceview(SDK_FishEyeFrame fishEyeFrame) {
		if ( null == fishEyeFrame ) {
			if ( null == mSufaceView
				...
				if(mOnSwitchViewListener!=null) mOnSwitchViewListener.switchFishEyes(false);
				// 切换到普通视频播放
...
//					|| !(mSufaceView instanceof VRSoftGLView)) {
				...
				if (fishEyeFrame instanceof SDK_FishEyeFrameSW) {
					if(mOnSwitchViewListener!=null) mOnSwitchViewListener.switchFishEyes(true);
...
//					}
					...
					//设置形态
//					((VRSoftGLView) mSufaceView).setShape(VRSoftDefine.XMVRShape.Shape_Ball_Hat);
//					//设置位置
...
//			}
	...
	public void setType(int Shape){
		((VRSoftGLView) mSufaceView).setShape(Shape);
	}
...
//                FunSDK.MediaSetFluency(mPlayerHandler, SDKCONST.EDECODE_TYPE.EDECODE_REAL_TIME_STREAM0, 0); //设置流畅度（实时<-->流畅）
	...
	public String captureImage(String path) {
		if ( 0 != mPlayerHandler ) {
			if ( null == path ) {
				...
				path = FunPath.getSigleCapturePath(path);
			}
	 ...
	 * @param mDeviceSn
	 */
	public void startRecordVideo(String mDeviceSn) {
		if (0 != mPlayerHandler) {
			...
			if (!bRecord) {
				...
				mFilePath = FunPath.getRecordPath(mDeviceSn);
				bRecord = true;
...
//				}
	...
	public String autocaptureImage(@NotNull String path){
		if(0!=mPlayerHandler){
	...
	public interface SwitchViewListener{
		void switchFishEyes(boolean flag);
	}
