...
public class ProtractorView extends View {
    ...
    private Paint centerTextHint;

    //Arc related dimens
    private int mArcRadius = 0;
    private int mArcWidth = 6;
    private int mArcProgressWidth = 6;
    ...
    private int mcenterTextSize = 100;
    ...
    private float x_centertext;
    private float y_centertext;
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
         ...
         */
        int arcColor = res.getColor(R.color.nonChose);
        int arcProgressColor = res.getColor(R.color.white);
        int textColor = res.getColor(R.color.text_color);
        int textProgressColor = res.getColor(R.color.white);
        int tickColor = res.getColor(R.color.trans_white);
        int tickProgressColor = res.getColor(R.color.white);
        ...
        centerTextHint = new Paint();
        centerTextHint.setColor(textProgressColor);
        centerTextHint.setAntiAlias(true);
        centerTextHint.setStyle(Paint.Style.FILL);
        centerTextHint.setTextSize(mcenterTextSize);
        centerTextHint.setTextAlign(Paint.Align.CENTER);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mArcRect, 0, MAX, false, mArcProgressPaint);
        canvas.drawArc(mArcRect, 0, mAngle, false, mArcPaint);
        ...
        for (int i = 360; i >= 180; i -= mTickIntervals) {
            ...
            if (count == mTicksBetweenLabel.ordinal()) {
                ...
                canvas.drawText("" + (i - 180)*100/180+"%", (float) midTickX, (float) midTickY, (mAngle <= 360 - i) ?  mTickTextColoredPaint:mTickTextPaint );
                count = 0;
            } else {
                //for tick
                canvas.scale(-1, 1, mArcRect.centerX(), mArcRect.centerY());
        ...
        canvas.drawText("" + (180 - mAngle)*100/180+"%", x_centertext, y_centertext,  centerTextHint);
