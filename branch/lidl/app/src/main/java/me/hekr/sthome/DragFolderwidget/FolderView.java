...
public class FolderView extends RelativeLayout {
    ...
    private void init(LayoutCalculator lc, int screenHeight) {
        ...
        LayoutParams params = new LayoutParams((lc.iconWidth + lc.gapH2) * 3, screenHeight - lc.dpToPixel(40) - 4
                * lc.folderMargin);
        //居中
    ...
    public void onReday(LayoutCalculator lc, ObjectPool pp, int screenHeight) {
        ...
        mFolderName.setText(mInfo.getText());
     ...
     * @param commit
     */
    public void setEditingEnable(boolean commit) {
...
//
//        mFolderName.setFocusable(commit);
//        mFolderName.setEnabled(commit);
    }
    ...
    public String getEditContent(){
        return mFolderName.getText().toString().trim();
    }
    ...
    public void refresh(FolderInfo mInfo){
        if(mContentView!=null){
            mContentView.setFolderInfo(mInfo);
